...
1. Check `node --version`, `agent-browser --version`, and `gh --version`. Install a supported Node.js release through the existing runtime manager if Node is missing.
2. If `agent-browser` is missing, use `brew install agent-browser` on a Homebrew-managed host, or `npm install -g agent-browser` otherwise. Run `agent-browser install` to install its browser when no compatible browser is available. On Linux, use `agent-browser install --with-deps` if browser system libraries are missing.
3. GitHub CLI 2.99+ is required for attachments. If `gh` is missing, install it through the host's package manager, such as `brew install gh`. If it lacks attachment support, upgrade it through the manager that installed it, such as `brew upgrade gh`. For other platforms, follow the official [GitHub CLI installation instructions](https://github.com/cli/cli#installation).
4. Verify `agent-browser skills get core --full` works. If an older installation lacks bundled skills, run `agent-browser upgrade` and retry. Confirm both `gh pr edit --help` and `gh pr comment --help` list `--attach`; a version number alone is not enough.
5. Check `gh auth status` before publishing. Preserve existing accounts, credential helpers, and Git configuration. Do not log in, switch accounts, or change global authentication automatically. Ask for user action if authentication is missing.

Respect host installation restrictions. If setup requires elevated privileges or approval, request it rather than bypassing the restriction. Recheck the failed prerequisite after setup. If installation or verification still fails, report the exact blocker and missing evidence; do not claim capture or publication succeeded.
