 ...
 *
 * After a compaction the agent otherwise sits idle waiting for a new prompt.
 * This injects a follow-up that makes it recover intent from the session JSONL
 * and resume the unfinished step.
 *
...
1. Review the active session branch leading to the compaction entry. Focus first on messages and tool calls immediately before compaction, searching earlier history only as needed. JSONL append order can include abandoned branches, so follow parentId links rather than treating every entry as active.
...
3. Reconcile the recovered history with the compaction summary and current repository state. Treat the current worktree as authoritative for file state and the session history as authoritative for user intent.
